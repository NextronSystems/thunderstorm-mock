/*
 * THOR Thunderstorm API - Unit Tests (oapi-codegen version)
 *
 * This file contains unit tests for the Thunderstorm mock server.
...
func testRouter() *httptest.Server {
	...
	handler := HandlerWithOptions(mockServer, StdHTTPServerOptions{
		BaseURL: "/api/v1",
...
func TestCheckEndpoint(t *testing.T) {
	...
	if len(report) > 0 {
		...
		if finding["type"] != "THOR Finding" {
			t.Errorf("expected finding type 'THOR Finding', got '%v'", finding["type"])
		}
...
func TestCheckEndpointWithSource(t *testing.T) {
	...
	if len(report) > 0 && report[0]["source"] != "custom-source" {
		t.Errorf("expected source 'custom-source', got '%v'", report[0]["source"])
	}
}
...
func TestFullAsyncWorkflow(t *testing.T) {
	...
	var tsStatus ThunderstormStatus
	_ = json.Unmarshal(statusBody, &tsStatus)

	if tsStatus.ScannedSamples < 1 {
...
func TestCheckEndpointMissingFile(t *testing.T) {
	...
	// Should return an error (400 for missing required field)
	if resp.StatusCode != http.StatusBadRequest {
		t.Errorf("expected status 400, got %d", resp.StatusCode)
	}
}
